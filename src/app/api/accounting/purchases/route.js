...
export async function PUT(req) {
    try {
        ...
        await getTenantCollection(typeof empresaId !== 'undefined' ? empresaId : '6', 'purchases_ledger').doc(id).update(update);
...
export async function DELETE(req) {
    try {
        ...
        if (!id) return Response.json({ error: 'id requerido' }, { status: 400 });
        await getTenantCollection(typeof empresaId !== 'undefined' ? empresaId : '6', 'purchases_ledger').doc(id).delete();
        return Response.json({ success: true });
    } catch (err) {
