...
export async function GET(req) {
    try {
        ...
        let q = getTenantCollection(typeof empresaId !== 'undefined' ? empresaId : 'ayatech', 'sales_ledger').where('companyProfileId', '==', companyProfileId);
...
export async function POST(req) {
    try {
        ...
        const ref = await getTenantCollection(typeof empresaId !== 'undefined' ? empresaId : 'ayatech', 'sales_ledger').add(data);
...
export async function PUT(req) {
    try {
        ...
        await getTenantCollection(typeof empresaId !== 'undefined' ? empresaId : 'ayatech', 'sales_ledger').doc(id).update(update);
...
export async function DELETE(req) {
    try {
        ...
        if (!id) return Response.json({ error: 'id requerido' }, { status: 400 });
        await getTenantCollection(typeof empresaId !== 'undefined' ? empresaId : 'ayatech', 'sales_ledger').doc(id).delete();
        return Response.json({ success: true });
    } catch (err) {
