...
export async function GET(req) {
    try {
        ...
        let payload;
        if (libro === '14.1') {
            const snap = await getTenantCollection(typeof empresaId !== 'undefined' ? empresaId : 'ayatech', 'sales_ledger')
            ...
            const snap = await getTenantCollection(typeof empresaId !== 'undefined' ? empresaId : 'ayatech', 'purchases_ledger')
                .where('companyProfileId', '==', companyProfileId).where('period', '==', period).get();
