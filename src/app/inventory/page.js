...
export default function InventoryDashboard() {
    const { user } = useAuth()
    // Campos y etiquetas propios del rubro de esta empresa (aceros, comestibles…).
    const { config } = useInventoryConfig(user?.empresaId)
    ...
    // Valores de los campos configurables del rubro (van al mapa `attributes`).
    ...
    useEffect(() => {
        if (user?.empresaId) {
            ...
            // Sin empresa asignada no hay inventario que mostrar.
            setLoading(false)
        }
    }, [user?.empresaId])
    ...
    const fetchInventory = async () => {
        ...
        setLoading(true)
        try {
            const res = await fetch(`/api/inventory?empresaId=${encodeURIComponent(user.empresaId)}`)
            const data = await res.json()
            if (Array.isArray(data)) {
                setItems(data)
            } else {
                console.error('Error de API (inventario):', data?.error)
                setItems([])
            }
        } catch (error) {
    ...
    const handleOpenModal = (item = null) => {
        ...
        if (item) {
            ...
            setFormData({
                ...
                unit: item.unit || config.unitOptions[0] || 'Unidades',
                cost: item.cost || 0,
    ...
    const handleChange = (e) => {
        ...
        setFormData(prev => {
            ...
            // Auto-generar SKU si cambia la categoría y estamos creando un ítem nuevo
            if (name === 'category' && !editingItem) {
                const catStr = value.trim();
                if (catStr.length >= 3) {
                    ...
                    const oldPrefix = prev.category ? prev.category.trim().substring(0, 3).toUpperCase() : '';
                    if (!prev.sku || (oldPrefix.length >= 3 && prev.sku.startsWith(`${oldPrefix}-`))) {
                        nextData.sku = `${prefix}-${(maxNum + 1).toString().padStart(3, '0')}`;
                    }
                } else if (catStr.length < 3) {
                    const oldPrefix = prev.category ? prev.category.trim().substring(0, 3).toUpperCase() : '';
                    if (oldPrefix.length >= 3 && prev.sku.startsWith(`${oldPrefix}-`)) {
                        nextData.sku = '';
                    }
                }
            }
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const payload = { ...formData, attributes, imageUrl: finalImageUrl, empresaId: user?.empresaId || null };
            if (editingItem) {
    ...
    // Columnas extra de la tabla, según el rubro configurado para la empresa.
    const extraColumns = useMemo(
        () => config.listColumns.map(key => config.extraFields.find(f => f.key === key)).filter(Boolean),
        [config]
    )
            ...
            <div style={{ display: 'flex', flexDirection: 'column', minHeight: '100vh', background: '#f8fafc' }}>
                ...
                <main style={{ flex: 1, padding: '2rem', maxWidth: '1400px', margin: '0 auto', width: '100%' }}>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
                        <div>
                            <h1 style={{ margin: '0 0 0.5rem 0', fontSize: '2rem', color: '#0f172a', fontWeight: 700 }}>Inventario</h1>
                            <p style={{ margin: 0, color: '#64748b' }}>Gestiona los niveles de stock, costos y alertas del almacén.</p>
                        </div>
                        <div style={{ display: 'flex', gap: '1rem' }}>
                            <button 
                                ...
                                style={{
                                    ...
                                    padding: '0.75rem 1.25rem', borderRadius: '8px', fontWeight: 600, cursor: 'pointer',
                                    boxShadow: '0 1px 2px rgba(0,0,0,0.05)'
                                }}
                            >
                                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                                    <path d="M4 7V4h3M20 7V4h-3M4 17v3h3M20 17v3h-3M9 9h6M9 12h6M9 15h6"></path>
                                </svg>
                        ...
                        <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap', marginBottom: '1.5rem', alignItems: 'center', justifyContent: 'space-between' }}>
                            <div style={{ position: 'relative', flex: '1 1 300px', maxWidth: '500px' }}>
                                ...
                                <input 
                                    ...
                                    style={{ width: '100%', padding: '0.8rem 1rem 0.8rem 2.5rem', borderRadius: '10px', border: '1px solid #cbd5e1', fontSize: '0.95rem' }}
                                />
                            </div>
                            ...
                            {!showFolders && (
                                <button 
                                    ...
                                    style={{ 
                                        display: 'flex', alignItems: 'center', gap: '0.5rem',
                                        background: '#f1f5f9', border: '1px solid #e2e8f0', padding: '0.6rem 1rem', 
                        ...
                        {showFolders ? (
                                ...
                                {Object.entries(categoryStats).filter(([cat]) => cat !== 'Todas').sort((a, b) => b[1] - a[1]).map(([cat, count]) => (
                                    ...
                                    >
                                        ...
                                        <div style={{ textAlign: 'center' }}>
                                            <h3 style={{ margin: '0 0 0.25rem 0', fontSize: '1.1rem', color: '#1e293b' }}>{cat}</h3>
                                            <span style={{ fontSize: '0.85rem', color: '#64748b', fontWeight: 600 }}>{count} ítems</span>
                                        </div>
                                    </div>
                                ))}
                            </div>
                            ...
                            <div style={{ overflowX: 'auto' }}>
                                {categoryFilter && !searchQuery && (
                                    <h2 style={{ fontSize: '1.2rem', color: '#0f172a', marginBottom: '1rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                        ...
                        <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', minWidth: '700px' }}>
                            <thead>
                                <tr style={{ borderBottom: '2px solid #e2e8f0' }}>
                                    ...
                                    <th style={{ padding: '1rem 0.5rem', color: '#64748b', fontWeight: 600, fontSize: '0.85rem', textTransform: 'uppercase' }}>{coreLabel(config, 'category', 'Categoría')}</th>
                                    {extraColumns.map(f => (
                                        <th key={f.key} style={{ padding: '1rem 0.5rem', color: '#64748b', fontWeight: 600, fontSize: '0.85rem', textTransform: 'uppercase' }}>{f.label}</th>
                                    ))}
                            ...
                            <tbody>
                                {loading ? (
                                    <tr>
                                        <td colSpan={totalColumns} style={{ textAlign: 'center', padding: '3rem', color: '#64748b' }}>Cargando inventario...</td>
                                    </tr>
                                ) : filteredItems.length === 0 ? (
                                    <tr>
                                        <td colSpan={totalColumns} style={{ textAlign: 'center', padding: '3rem', color: '#64748b' }}>No se encontraron registros de inventario.</td>
                                    </tr>
                                ) : (
                                    filteredItems.map(item => (
                                        <tr key={item.id} style={{ borderBottom: '1px solid #f1f5f9', transition: 'background 0.2s' }} onMouseOver={e => e.currentTarget.style.background = '#f8fafc'} onMouseOut={e => e.currentTarget.style.background = 'transparent'}>
                                            ...
                                            {extraColumns.map(f => (
                                                <td key={f.key} style={{ padding: '1rem 0.5rem', color: '#475569', fontSize: '0.875rem' }}>
                                                    {formatFieldValue(f, item.attributes?.[f.key])}
                                                </td>
                                            ))}
                        ...
                        <div className="modal-content" style={{ background: '#fff', width: '100%', maxWidth: '500px', borderRadius: '12px', boxShadow: '0 20px 25px -5px rgba(0, 0, 0, 0.1), 0 10px 10px -5px rgba(0, 0, 0, 0.04)', display: 'flex', flexDirection: 'column', maxHeight: '90vh' }}>
                            ...
                            <form onSubmit={handleSubmit} style={{ padding: '1rem 1.5rem', overflowY: 'auto' }}>
                                <div style={{ display: 'grid', gap: '0.75rem' }}>
                                    ...
                                    <div>
                                        <label style={{ display: 'block', fontSize: '0.85rem', fontWeight: 600, color: '#475569', marginBottom: '0.4rem' }}>{coreLabel(config, 'itemName', 'Nombre del Material / Producto')} *</label>
                                        <input 
                                            type="text" 
                                            name="name" 
                                            value={formData.name} 
                                    ...
                                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem' }}>
                                        <div>
                                            <label style={{ display: 'block', fontSize: '0.85rem', fontWeight: 600, color: '#475569', marginBottom: '0.4rem' }}>{coreLabel(config, 'sku', 'SKU / Código')}</label>
                                            <input 
                                                type="text" 
                                                name="sku" 
                                                value={formData.sku} 
                                    ...
                                    <div>
                                        <label style={{ display: 'block', fontSize: '0.85rem', fontWeight: 600, color: '#475569', marginBottom: '0.4rem' }}>{coreLabel(config, 'cost', 'Costo Unitario Referencial (S/)')}</label>
                                        <input
                                            type="number"
                                            name="cost"
                                            value={formData.cost}
